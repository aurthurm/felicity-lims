...
export function useReflexValidation(config?: Partial<ValidationConfig>) {
    ...
    const validateTriggerNode = (node: Node): NodeValidationResult => {
        ...
        if (node.data.description && node.data.description.trim().length < 10) {
            nodeWarnings.push({
                nodeId: node.id,
                type: 'warning',
                severity: 'low',
     ...
     */
    const validateConnections = (nodes: Node[], edges: Edge[]): ValidationIssue[] => {
        ...
        for (const edge of edges) {
            ...
            if (sourceType === 'decision') {
                ...
                const validHandleConnection =
                    (sourceHandle === 'rules' && targetType === 'rule') || (sourceHandle === 'actions' && targetType === 'action');
     ...
     */
    const updateNodeClasses = (nodes: Node[], validationResults: Map<string, NodeValidationResult>): Node[] => {
        return nodes.map(node => {
            ...
            if (allIssues.length > 0) {
                const mostSevere = allIssues.reduce((prev, curr) => (curr.severity > prev.severity ? curr : prev));
     ...
     */
    const debouncedValidate = useDebounceFn((nodes: Node[], edges: Edge[]) => validateGraph(nodes, edges), finalConfig.debounceMs);
    ...
    const getNodeIssues = (nodeId: string): ValidationIssue[] => {
        return [...errors.value, ...warnings.value].filter(issue => issue.nodeId === nodeId);
    };
    ...
    const getEdgeIssues = (edgeId: string): ValidationIssue[] => {
        return [...errors.value, ...warnings.value].filter(issue => issue.edgeId === edgeId);
    };
