...
export function useLaboratoryContextMiddleware() {
    ...
    const currentLaboratoryUid = computed(() => contextStore.context.activeLaboratory?.uid);

    const currentUserUid = computed(() => authStore.auth.user?.uid);

    const isContextValid = computed(() => Boolean(currentLaboratoryUid.value && currentUserUid.value));
    ...
    const addContextHeaders = (request: ContextAwareRequest = {}, requireLaboratoryContext = true): ContextAwareRequest => {
    ...
    const addContextVariables = (request: ContextAwareRequest = {}, requireLaboratoryContext = true): ContextAwareRequest => {
        const validation = validateContext(requireLaboratoryContext);
    ...
    const fetchWithContext = async (url: string, options: RequestInit = {}, requireLaboratoryContext = true): Promise<Response> => {
        try {
            const contextRequest = addContextHeaders(
                {
    ...
    const operationRequiresLaboratoryContext = (operationName?: string): boolean => {
        ...
        return !globalOperations.some(op => operationName.toLowerCase().includes(op.toLowerCase()));
    };

    // Context change listener for automatic request retry
    const retryRequestOnContextChange = (originalRequest: () => Promise<any>, maxRetries = 3) => {
        let retryCount = 0;
