...
type OperationResult = OperationError | OperationSuccess | null | undefined;
...
export default function useApiUtil() {
     ...
     */
    const gqlOperationalErrorHandler = <T extends Record<string, OperationResult>>(payload: T, key?: keyof T): T => {
     ...
     */
    const GQLResponseInterceptor = <T extends Record<string, any>>(res: any, key?: keyof T): T => {
        const processedResponse = gqlResponseHandler<T>(res);

        // Use type assertion to handle empty object case
        return key ? gqlOperationalErrorHandler({ [key]: processedResponse[key] } as T, key) : processedResponse;
    };
