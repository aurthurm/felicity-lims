...
function setupLaboratoryAnalytics() {
    ...
    const recordLaboratoryUsage = (laboratoryUid: string, duration: number) => {
        ...
        if (analytics) {
            analytics.usage_statistics.average_session_duration = (analytics.usage_statistics.average_session_duration + duration) / 2;
    ...
    const handleRealTimeUpdate = (update: any) => {
        switch (update.type) {
            ...
            case 'laboratory_updated':
                const updateIndex = laboratoryStore.store.laboratories.findIndex(lab => lab.uid === update.laboratory.uid);
                if (updateIndex > -1) {
            ...
            case 'laboratory_deleted':
                laboratoryStore.store.laboratories = laboratoryStore.store.laboratories.filter(lab => lab.uid !== update.laboratory_uid);
                break;
...
function setupAutoSwitch() {
    ...
    const autoSwitchToFrequent = async () => {
        ...
        if (!currentLab && availableLabs.length > 0) {
            ...
            const targetLab = mostUsed ? availableLabs.find(lab => lab.uid === mostUsed) : availableLabs[0];
...
function initializeLaboratoryPlugin() {
    ...
    window.dispatchEvent(
        new CustomEvent('laboratory-plugin-initialized', {
            detail: {
                version: '1.0.0',
                features: [
