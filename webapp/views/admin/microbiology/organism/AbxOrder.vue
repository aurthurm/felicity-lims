...
<template>
  <div class="space-y-6">
    <PageHeading title="Orders">
      <Button @click="FormManager(true)">Add Order</Button>
    </PageHeading>
        ...
        <Table class="w-full caption-bottom text-sm">
          <TableHeader class="[&_tr]:border-b">
            <TableRow class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
              ...
              <TableHead class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Class</TableHead>
          ...
          <TableBody class="[&_tr:last-child]:border-0">
            <TableRow v-for="guideline in abxOrders" :key="guideline?.uid" class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
              <TableCell class="px-4 py-3 align-middle whitespace-nowrap text-sm text-foreground">{{ guideline?.name }}</TableCell>
              <TableCell class="px-4 py-3 align-middle whitespace-nowrap text-sm text-foreground">{{ guideline?.class_?.name }}</TableCell>
            ...
            <TableEmpty v-if="!abxOrders || abxOrders.length === 0" :colspan="3">
              <Empty class="border-0 bg-transparent p-0">
                <EmptyContent>
                  <EmptyHeader>
                    <EmptyTitle>No orders found</EmptyTitle>
                    <EmptyDescription>Add an order to get started.</EmptyDescription>
                  </EmptyHeader>
                </EmptyContent>
              </Empty>
            </TableEmpty>
