...
const InventoryHome = defineComponent({
 ...
 {{
 header: () => (
 <span class="text-lg font-medium text-card-foreground">Order Basket</span>
 ),
 body: () => (
 <div class="space-y-4">
 ...
 <th class="text-right text-sm font-medium text-foreground">Quantity</th>
 <th class="text-right text-sm font-medium text-foreground">Actions</th>
 </tr>
 </thead>
 <tbody class="divide-y divide-border bg-background">
 {this.basket.map(item => (
 <tr key={item.product.uid}>
 <td class="whitespace-nowrap text-sm text-foreground">{item.product.name}</td>
 <td class="whitespace-nowrap text-sm text-right text-foreground">{item.quantity}</td>
