...
export const useLaboratoryContextStore = defineStore('laboratoryContext', () => {
    ...
    // Computed properties
    const hasMultipleLaboratories = computed(() => context.value.availableLaboratories.length > 1);
    ...
    const loadContextFromStorage = () => {
        try {
            ...
            if (stored) {
                ...
                if (contextData.activeLaboratoryUid && isLaboratoryAccessible.value(contextData.activeLaboratoryUid)) {
