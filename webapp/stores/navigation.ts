...
export const useNavigationStore = defineStore('navigation', () => {
    ...
    const categorizedNavigation = computed(() => {
        ...
        return categories.filter(cat => cat.items.length > 0).sort((a, b) => (a.order || 999) - (b.order || 999));
    });
    ...
    const filteredNavigation = computed(() => {
        ...
        return availableNavigationItems.value.filter(
            item =>
    ...
    const breadcrumbs = computed((): BreadcrumbItem[] => {
        ...
        pathSegments.forEach((segment, index) => {
            ...
            const navItem = availableNavigationItems.value.find(item => item.route === currentPath || item.route.startsWith(currentPath));
