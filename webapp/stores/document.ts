...
export const useDocumentStore = defineStore('documents', {
    ...
    actions: {
        ...
        updateDocument(document: DocumentType): void {
            ...
            this.documents = this.documents.map(doc => (doc.uid === document.uid ? updatedDoc : doc));

            this.recentDocuments = [updatedDoc, ...this.recentDocuments.filter(doc => doc.uid !== document.uid)].slice(0, 5);
        },
        ...
        updateFolder(folder: DocumentFolderType): void {
            ...
            this.folders = this.folders.map(f => (f.uid === folder.uid ? folder : f));
        },
    ...
    getters: {
        getChildFolders:
            state =>
            (parentUid: string | null): DocumentFolderType[] => {
                return state.folders.filter(folder => folder.parentUid === parentUid);
            },

        getDocumentsInFolder:
            state =>
            (folderUid: string): DocumentType[] => {
