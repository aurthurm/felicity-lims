...
watch(
  ...
  (curr, _) => {
    if (curr !== true) {
      ...
      setTimeout(function () {
        checdiv.value?.classList.add("hidden");
      }, 500);
    } else {
      setTimeout(function () {
      ...
      checdiv.value?.classList.remove("hidden");
    }
  }
);
</script>
...
<template>
  ...
  >
    <template #header>
      <span class="text-lg font-medium uppercase text-card-foreground">Notifications</span>
    </template>
    ...
    <template #body>
      ...
      >
        <div
          class="rounded-full bg-muted p-5 mb-4"
          aria-hidden="true"
        >
          <svg
            width="32"
        ...
        <p class="text-sm text-muted-foreground mt-1 max-w-[220px]">
          Activity from samples, worksheets, and reports will appear here when available.
        </p>
      </div>
      ...
      <template v-else>
        ...
        >
          <div
            class="w-6 h-6 border rounded-full border-border flex items-center justify-center bg-primary/10 shrink-0"
            aria-hidden="true"
          >
            <svg
              width="16"
