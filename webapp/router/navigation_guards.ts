 ...
 */
export const laboratoryContextGuard = async (to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) => {
    ...
    const requiresLaboratory =
        navItem?.laboratorySpecific ||
 ...
 */
export const permissionGuard = async (to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) => {
    const authStore = useAuthStore();
    const navigationStore = useNavigationStore();
 ...
 */
export const navigationTrackingGuard = async (to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) => {
    const navigationStore = useNavigationStore();
 ...
 */
export const adminGuard = async (to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) => {
    const authStore = useAuthStore();
 ...
 */
export const laboratoryParameterGuard = async (to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) => {
    const contextStore = useLaboratoryContextStore();
    ...
    if (laboratoryUid) {
        ...
        if (!contextStore.context.activeLaboratory || contextStore.context.activeLaboratory.uid !== laboratoryUid) {
            try {
...
export const routeRequiresLaboratory = (routePath: string): boolean => {
    const laboratorySpecificPaths = ['/dashboard', '/samples', '/worksheets', '/quality-control', '/bio-banking'];
