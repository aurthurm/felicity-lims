...
async def requisite_setup(
        org_name: str = "Beak Labs", lab_name: str = "My First Laboratory"
) -> bool:
...
async def initialize_beak() -> bool:
    logger.info("Initializing Beak LIMS ...")
    await requisite_setup()
    await default_setup()
    logger.info("Beak LIMS Initialisation completed.")
    return True
