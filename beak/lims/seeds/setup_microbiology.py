...
from beak.apps.multiplex.microbiology.services import (
    ...
    AbxGuidelineYearService,
)
from beak.core.config import get_settings
from beak.lims.seeds.data import get_whonet_dataframes
...
async def seed_antibiotics() -> None:
    ...
    data = _clean_df(
        get_whonet_dataframes("Antibiotics"),
        rename={"antibiotic": "name", "class": "class_"},
    )
    ...
    for guideline in guidelines:
        ...
        if not _guideline:
            guideline_in = AbxGuidelineCreate(
                name=guideline, code=guideline, description=guideline
            )
    ...
    for _, item in data.iterrows():
        antibiotic_in = {
            key: item[key] for key in AbxAntibioticCreate.model_fields if key in item
        }
        # only seed human abx
        ...
        antibiotic_in["veterinary"] = (
            True if antibiotic_in["veterinary"] == "X" else False
        )
        antibiotic_in["guidelines"] = item.get("guidelines", [])

        if not (
                await abx_antibiotic_service.get(
                    name=antibiotic_in["name"], abx_number=str(antibiotic_in["abx_number"])
                )
        ):
            ...
            for gl_name, gl in guides.items():
                if (
                        antibiotic_in["guidelines"]
                        and gl_name in antibiotic_in["guidelines"]
                ):
                    gl_in = AbxAntibioticGuidelineCreate(
                        antibiotic_uid=_abx_antibiotic.uid, guideline_uid=gl.uid
                    )
...
async def seed_organisms():
    ...
    data = _clean_df(
        get_whonet_dataframes("Organisms"),
        rename={"organism": "name", "class": "class_"},
    )
        ...
        for phylum in king_sub["phylum"].unique():
            ...
            for _class in phylum_sub["class_"].unique():
                ...
                if cl is None and _class != "":
                    cl_in = AbxClassCreate(
                        name=_class, phylum_uid=ph.uid if ph else None
                    )
                ...
                for order in class_sub["order"].unique():
                    od = (
                        await AbxOrderService().get(name=order) if order != "" else None
                    )
                    if od is None and order != "":
                        od_in = AbxOrderCreate(
                            name=order, class_uid=cl.uid if cl else None
                        )
                    ...
                    for family in order_sub["family"].unique():
                        fa = (
                            await AbxFamilyService().get(name=family)
                            if family != ""
                            else None
                        )
                        if fa is None and family != "":
                            fa_in = AbxFamilyCreate(
                                name=family, order_uid=od.uid if od else None
                            )
                        ...
                        for genus in family_sub["genus"].unique():
                            ge = (
                                await AbxGenusService().get(name=genus)
                                if genus != ""
                                else None
                            )
                            if ge is None and genus != "":
                                ge_in = AbxGenusCreate(
                                    name=genus, family_uid=fa.uid if fa else None
                                )
                            ...
                            for _, item in genus_sub.iterrows():
                                ...
                                if not (
                                        await AbxOrganismService().get(
                                            name=organism_in["name"],
                                            whonet_org_code=organism_in["whonet_org_code"],
                                        )
                                ):
...
async def seed_organism_serotypes():
    ...
    for _, item in data.iterrows():
        ...
        org = [
            x
            for x in organisms
            if x and getattr(x, "whonet_org_code", None) == item["organism"]
        ]
        bp_in = {
            **bp_in,
        ...
        if not (
                await AbxOrganismSerotypeService().get(
...
async def seed_breakpoints():
    ...
    guideline_years = []
    unique_gl_ys = (
        data[["guidelines", "year"]]
        .drop_duplicates()
        .sort_values(by=["guidelines", "year"])
    )
    for g_y in unique_gl_ys.itertuples(index=False):
        gl = list(filter(lambda x: x.name == g_y.guidelines, guidelines))[0]
        gl_year = await AbxGuidelineYearService().get(
            guideline_uid=gl.uid, year=g_y.year
        )
        if not gl_year:
            gl_year_in = AbxGuidelineYearCreate(
                guideline_uid=gl.uid, year=g_y.year, code=f"{gl.name} {g_y.year}"
            )
    ...
    for _, item in data.iterrows():
        ...
        if item["host"] and not item["host"].lower().strip() == "human":
            continue

        bp_in = {
            key: item[key] for key in AbxBreakpointCreate.model_fields if key in item
        }
        gl = [
            x
            for x in guideline_years
            if x and getattr(x, "code", None) == f"{item['guidelines']} {item['year']}"
        ]
        bpt = [
            x
            for x in breakpoint_types
            if x and getattr(x, "name", None) == item["breakpoint_type"]
        ]
        hst = [x for x in hosts if x and getattr(x, "name", None) == item["host"]]
        soi = [
            x
            for x in site_of_infections
            if x and getattr(x, "name", None) == item["site_of_infection"]
        ]
        tm = [
            x
            for x in test_methods
            if x and getattr(x, "name", None) == item["test_method"]
        ]
        bp_in = {
            **bp_in,
        ...
        if not (
                await AbxBreakpointService().get(
...
async def seed_expected_resistance_phenotypes():
    ...
    for _, item in data.iterrows():
        ...
        gl = [
            x for x in guidelines if x and getattr(x, "name", None) == item["guideline"]
        ]
        rt_in = {
            **rt_in,
        ...
        if not (
                await AbxExpResPhenotypeService().get(
...
async def seed_expert_interpretation_rules():
    ...
    for _, item in data.iterrows():
        ...
        if not (
                await AbxExpertInterpretationRuleService().get(
...
async def seed_qc_ranges():
    ...
    for _, item in data.iterrows():
        rt_in = {key: item[key] for key in AbxQCRangeCreate.model_fields if key in item}
        gl = [
            x for x in guidelines if x and getattr(x, "name", None) == item["guideline"]
        ]
...
async def seed_ast_services():
    ...
    if not category:
        category_in = AnalysisCategoryCreate(
            name=category_name, description=category_name, active=True
        )
    ...
    analyses = [
        {
            "name": "Organisms",
            "description": "Isolated Organisms, There can be one or many organisms",
            "category_uid": category.uid,
            "ucuum": "",
            "keyword": "beak_ast_abx_organism",
            "sort_key": 1,
            "active": 1,
            "mappins": [{"loinc": ""}],
        },
        {
            "name": "Antibiotic",
            "description": "Beak Abx Antibiotics",
            "category_uid": category.uid,
            "ucuum": "",
            "keyword": "beak_ast_abx_antibiotic",
            "sort_key": 1,
            "active": 1,
            "mappins": [{"loinc": ""}],
        },
