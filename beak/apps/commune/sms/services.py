from beak.apps.analysis.entities.analysis import AnalysisSpecification, Sample
from beak.apps.analysis.entities.results import AnalysisResult
from beak.apps.setup.caches import get_laboratory
import logging

from beak.apps.abstract.service import BaseService
from beak.apps.commune.sms.entities import SmsMessage, SmsTemplate
from beak.apps.commune.sms.enum import SmsAudience, SmsStatus, SmsTrigger
from beak.apps.commune.sms.repository import (
    SmsMessageRepository,
    SmsTemplateRepository,
)
from beak.apps.commune.sms.schemas import (
    SmsMessageCreate,
    SmsMessageUpdate,
    SmsTemplateCreate,
    SmsTemplateUpdate,
)
from beak.utils.helpers import extract_template_variables
...
class SmsTemplateService(
    BaseService[SmsTemplate, SmsTemplateCreate, SmsTemplateUpdate]
):
    def __init__(self):
...
class SmsMessageService(BaseService[SmsMessage, SmsMessageCreate, SmsMessageUpdate]):
    ...
    async def create_sms_for_sample(self, uid: str) -> None:
        ...
        for result in sample.analysis_results:
            ...
            for template in templates:
                ...
                if await self._should_trigger_sms(
                    sample, result, template.specification_trigger
                ):
    ...
    async def _create_sms_for_template(self, sample, result, template):
        ...
        try:
            ...
            if template.audience == SmsAudience.PATIENT:
                ...
                if (
                    not client_contact
                    or not client_contact.phone_mobile
                    or not client_contact.consent_sms
                ):
            ...
            # Log the error
            print(
                f"Error creating SMS for result {result.uid} with template {template.uid}: {str(e)}"
            )
    ...
    async def render_sms(self, template: str, metadata: dict) -> str:
        ...
        try:
            ...
            if missing_vars:
                raise ValueError(
                    f"SMS Variables not found in metadata: {', '.join(missing_vars)}"
                )
    ...
    async def _should_trigger_sms(
        self, sample: Sample, result: AnalysisResult, trigger_type: SmsTrigger
    ) -> bool:
        """
        ...
        if isinstance(value, float):
            if trigger_type == SmsTrigger.NORMAL:
                return (spec.min is None or value >= spec.min) and (
                    spec.max is None or value <= spec.max
                )
            elif trigger_type == SmsTrigger.BELOW_NORMAL:
                return (
                    spec.min is not None
                    and value < spec.min
                    and (spec.min_warn is None or value > spec.min_warn)
                )
            elif trigger_type == SmsTrigger.ABOVE_NORMAL:
                return (
                    spec.max is not None
                    and value > spec.max
                    and (spec.max_warn is None or value < spec.max_warn)
                )
    ...
    async def _get_specification_for_result(
        self, sample: Sample, result: AnalysisResult
    ) -> AnalysisSpecification | None:
        """Get the appropriate specification for this result"""
        # Query specifications for this analysis
        specs = await AnalysisSpecificationService().get_all(
        ...
        for spec in specs:
            ...
            if (
                spec.age_min is not None
                and patient_age is not None
                and patient_age < spec.age_min
            ):
                continue

            if (
                spec.age_max is not None
                and patient_age is not None
                and patient_age > spec.age_max
            ):
                continue
