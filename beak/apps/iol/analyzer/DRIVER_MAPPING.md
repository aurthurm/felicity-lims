...
The IOL Analyzer module now supports **configurable JSON drivers** for extracting required fields from ASTM/HL7
messages. This eliminates the need for hardcoded parsers for 100+ different instruments.
...
### Method:
`transform_message(raw_message: str, laboratory_instrument_uid: str, lab_instrument_service, instrument_service) -> dict`
...
from beak.apps.instrument.services import LaboratoryInstrumentService, InstrumentService
...
| File                                             | Changes                                                                        |
|--------------------------------------------------|--------------------------------------------------------------------------------|
| `beak/apps/instrument/entities.py`               | Added `driver_mapping: Column(JSON)` to Instrument and LaboratoryInstrument    |
| `beak/apps/instrument/schemas.py`                | Added `driver_mapping: dict \| None` to all instrument schemas                 |
| `beak/apps/iol/analyzer/services/transformer.py` | Added 3 new methods: `extract_fields()`, `get_driver()`, `transform_message()` |

### Database

| File                                                                            | Changes                               |
|---------------------------------------------------------------------------------|---------------------------------------|
| `beak/migrations/versions/2025_10_27_1530-add_driver_mapping_to_instruments.py` | Migration: Add driver_mapping columns |
...
The driver mapping implementation provides a **configuration-based, zero-hardcoding approach** to extracting fields from
ASTM/HL7 messages. It's fully **async, production-ready, and tested**, with comprehensive error handling and logging
throughout.
