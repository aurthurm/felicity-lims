...
from beak.modules.core.api.gql.shipment import types
from beak.modules.core.api.gql.shipment.permissions import CanActionShipment
from beak.modules.core.api.gql.shipment.types import ShipmentType
...
class ShipmentMutations:
    ...
    async def action_shipment(self, info, uid: str, action: str) -> ShipmentResponse:  # noqa
        ...
        if action == "dispatch":
            shipment = await ShipmentService().change_state(
                shipment.uid, ShipmentState.AWAITING, beak_user.uid
            )
            ...
            shipment = await action_shipment(uid, action, beak_user)
    ...
    async def shipment_manage_samples(
        ...
        if payload.action == "assign":
            ...
            await shipment_recover(shipment.uid, data, beak_user.uid)
        elif payload.action == "recall":
            await shipment_recall(shipment.uid, data, beak_user.uid)
        else:
            pass
