...
from beak.modules.core.api.gql.messaging.types import MessageType
from beak.modules.shared.api.gql.permissions import IsAuthenticated
from beak.modules.shared.api.gql.types import DeletedItem, DeleteResponse, OperationError
...
class MessageMutations:
    ...
    async def view_message(self, info, uid: str) -> MessageResponse:
        ...
        message = await MessageService().view_message(message.uid, beak_user)
        return MessageType(**message.marshal_simple())
    ...
    async def delete_message(self, info, uid: str) -> DeleteResponse:
        ...
        uid = await MessageService().delete_for_user(message.uid, beak_user)
    ...
    async def delete_thread(self, info, uid: str) -> DeleteResponse:
        ...
        uid = await MessageThreadService().delete_for_user(thread.uid, beak_user)
        return DeletedItem(uid=uid)
