...
from beak.modules.core.api.gql.worksheet.permissions import CanActionWorksheet
from beak.modules.core.api.gql.worksheet.types import WorkSheetTemplateType, WorkSheetType
...
class WorkSheetMutations:
    ...
    @strawberry.mutation(
        extensions=[
            PermissionExtension(
                permissions=[
                    IsAuthenticated(),
                    HasPermission(FAction.UPDATE, FObject.WORKSHEET),
                ]
            )
        ]
    )
    ...
    async def action_worksheets(
        self, info, uids: list[str], action: str
    ) -> WorkSheetsResponse:
        beak_user = await auth_from_info(info)
        ...
        for ws_uid in uids:
            ...
            worksheet = await action_fun(ws_uid, beak_user)
