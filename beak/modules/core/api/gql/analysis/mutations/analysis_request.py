...
from beak.modules.core.api.gql.analysis.permissions import CanVerifySample
from beak.modules.core.api.gql.analysis.types import analysis as a_types
from beak.modules.core.api.gql.analysis.types import results as r_types
from beak.modules.shared.api.gql.auth import auth_from_info
from beak.modules.shared.api.gql.decorators import require_tenant_context
from beak.modules.shared.api.gql.permissions import IsAuthenticated, HasPermission
from beak.modules.shared.api.gql.types import (
    OperationError,
    OperationSuccess,
)
...
from beak.modules.core.analysis.services.creation_provider import (
    ...
    get_analysis_request_creation_provider,
)
...
async def create_analysis_request(
    ...
    try:
        analysis_request = await get_analysis_request_creation_provider().create_analysis_request(
            payload=payload, actor=beak_user
        )
    except AnalysisRequestCreationError as exc:
    ...
    for sample in analysis_request.samples:
        ...
        if lab_setting.auto_receive_samples:
            await SampleService().receive(sample.uid, received_by=beak_user)
...
async def derive_analysis_request(
    ...
    async with SampleService().repository.async_session() as transaction_session:
        ...
        async def _create_sample_with_tests(
            ...
            for _anal in analyses:
                if _anal.keyword == "beak_ast_abx_antibiotic":
                    continue
                await sample_service.repository.table_insert(
            ...
            for _a in created:
                if _a.keyword == "beak_ast_abx_organism":
    ...
    async with SampleService().repository.async_session() as transaction_session:
        ...
        for _, _sample in enumerate(to_clone):
            clone = await SampleService().clone_afresh(
                _sample.uid, beak_user, session=transaction_session
            )
...
async def cancel_samples(info, samples: List[str]) -> ResultedSampleActionResponse:
    beak_user = await auth_from_info(info)
    cancelled = await SampleWorkFlow().cancel(samples, cancelled_by=beak_user)
...
async def re_instate_samples(info, samples: List[str]) -> ResultedSampleActionResponse:
    ...
    for _sa_uid in samples:
        ...
        sample = await SampleWorkFlow().re_instate(
            sample.uid, re_instated_by=beak_user
        )
        if sample:
...
async def receive_samples(info, samples: List[str]) -> ResultedSampleActionResponse:
    ...
    for _sa_uid in samples:
        ...
        sample = await SampleWorkFlow().receive(sample.uid, received_by=beak_user)
        if sample:
            return_samples.append(sample)
...
async def verify_samples(info, samples: List[str]) -> SampleActionResponse:
    ...
    for _sa_uid in samples:
        ...
        _, sample = await SampleWorkFlow().approve(
            sample.uid, approved_by=beak_user
        )
        if sample:
            return_samples.append(sample)
    ...
    for _sam in samples:
        ...
        for re_uid in _sam.reasons:
            ...
            sample = await SampleWorkFlow().reject(
                sample.uid, rejected_by=beak_user
            )
                ...
                if sample.status == SampleState.REJECTED:
                    for analyte in sample.analysis_results:
                        await AnalysisResultWorkFlow().cancel(
                            analyte.uid, cancelled_by=beak_user
                        )
    ...
    if not_final:
        ...
        for sample in ns_samples:
            await ActivityStreamService().stream(
                sample, beak_user, sample.status, NotificationObject.SAMPLE
            )
...
async def print_samples(info, samples: List[str]) -> SampleActionResponse:
    ...
    for _sa_uid in samples:
        ...
        sample = await SampleWorkFlow().print(sample.uid, printed_by=beak_user)
        if sample:
            return_samples.append(sample)
    ...
    for _sa_uid in samples:
        async with SampleService().repository.async_session() as transaction_session:
            ...
            copy, invalidated = await SampleWorkFlow().invalidate(
                sample.uid, invalidated_by=beak_user, session=transaction_session
            )
...
async def samples_apply_template(
    ...
    if sample.status != SampleState.RECEIVED:
        await SampleService().change_status(
            sample.uid, status=SampleState.RECEIVED, updated_by_uid=beak_user.uid
        )
...
async def manage_analyses(
    ...
    for _anal in payload.cancel:
        ...
        await AnalysisResultWorkFlow().cancel(result.uid, cancelled_by=beak_user)
