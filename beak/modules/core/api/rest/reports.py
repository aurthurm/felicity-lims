...
async def request_report_generation(
    ...
    report = await report_service.create(
        data,
        related=["analyses", "created_by", "updated_by"],
    )
...
async def delete_report(
    ...
    try:
        ...
        message = (
            f"Error deleting File {report.location}. "
            "Report record deleted successfully."
        )
        logger.error("Error deleting file %s: %s", report.location, e)

    await report_service.repository.table_delete(analysis_reports, report_uid=report.uid)
