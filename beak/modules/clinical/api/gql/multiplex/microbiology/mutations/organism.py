...
from beak.modules.clinical.api.gql.multiplex.microbiology import (
    AbxKingdomType,
    AbxPhylumType,
    AbxClassType,
    AbxOrderType,
...
from beak.modules.clinical.api.gql.multiplex.microbiology.types import AbxOrganismResultType
from beak.modules.shared.api.gql.permissions import IsAuthenticated
from beak.modules.shared.api.gql.types import OperationError, DeletedItem, DeleteResponse
...
async def update_abx_kingdom(
    ...
    setattr(abx_kingdom, "updated_by_uid", beak_user.uid)
...
async def update_abx_phylum(
    ...
    setattr(abx_phylum, "updated_by_uid", beak_user.uid)
...
async def update_abx_class(
    ...
    setattr(abx_class, "updated_by_uid", beak_user.uid)
...
async def update_abx_order(
    ...
    setattr(abx_order, "updated_by_uid", beak_user.uid)
...
async def update_abx_family(
    ...
    setattr(abx_family, "updated_by_uid", beak_user.uid)
...
async def update_abx_genus(
    ...
    setattr(abx_genus, "updated_by_uid", beak_user.uid)
...
async def update_abx_organism(
    ...
    setattr(abx_organism, "updated_by_uid", beak_user.uid)
...
async def update_abx_organism_serotype(
    ...
    setattr(abx_serotype, "updated_by_uid", beak_user.uid)
...
@strawberry.mutation(permission_classes=[IsAuthenticated])
async def create_abx_organism_result(
        info,
    ...
    incoming = {
        "analysis_result_uid": analysis_result_uid,
        "isolate_number": len(existing) + 1 if existing else 1,
        "created_by_uid": beak_user.uid,
        "updated_by_uid": beak_user.uid,
    }

    obj_in = AbxOrganismResultCreate(**incoming)
    abx_org_res = await AbxOrganismResultService().create(obj_in)
...
async def save_abx_organism_result(
    ...
    await AnalysisResultService().update(
        analysis_result.uid,
        {"result": ", ".join(organism_names), "updated_by_uid": beak_user.uid},
    )
    return AbxOrganismResultType(**abx_org_res.marshal_simple())


@strawberry.mutation(permission_classes=[IsAuthenticated])
async def remove_abx_organism_result(
        info,
        uid: str,
) -> DeleteResponse:
    abx_organism_result = await AbxOrganismResultService().get(uid=uid)
    ...
    await AbxOrganismResultService().delete(uid=uid)
