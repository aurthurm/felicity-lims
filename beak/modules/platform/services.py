...
class TenantProvisioningService:
    ...
    async def migrate(
        ...
        *,
        ...
        if module_scope and module_scope not in tenant.get("enabled_modules", []):
            raise ValueError(
                f"Module '{module_scope}' is not enabled for tenant '{slug}'"
            )
    ...
    async def list_modules(self, *, slug: str) -> dict:
        tenant = await self._get_tenant(slug)
        return {
            "tenant_slug": tenant["slug"],
            "primary_industry": tenant.get("primary_industry", "clinical"),
            "enabled_modules": tenant.get("enabled_modules", ["core", "clinical"]),
        }

    async def enable_module(self, *, slug: str, module_id: str) -> dict:
        tenant = await self._get_tenant(slug)
        selection = normalize_modules(
            primary_industry=tenant.get("primary_industry", "clinical"),
            enabled_modules=list(
                set(tenant.get("enabled_modules", ["core", "clinical"])) | {module_id}
            ),
        )
        await self.registry.set_modules(
            tenant["uid"],
            primary_industry=selection.primary_industry,
            enabled_modules=selection.enabled_modules,
        )
    ...
    async def disable_module(self, *, slug: str, module_id: str) -> dict:
        tenant = await self._get_tenant(slug)
        primary_industry = tenant.get("primary_industry", "clinical")
        if module_id in {"core", primary_industry}:
            raise ValueError(
                f"Module '{module_id}' cannot be disabled for tenant '{slug}'"
            )
        enabled = [m for m in tenant.get("enabled_modules", ["core", "clinical"]) if m != module_id]
        selection = normalize_modules(
            primary_industry=primary_industry,
