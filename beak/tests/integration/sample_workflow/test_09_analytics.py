...
async def test_generate_report(app_api, auth_data):
    time_now = timenow_dt()
...
async def test_fetch_reports(app_api, client_root, auth_data):
    response = await app_api.get("/reports", headers=auth_data["headers"])
