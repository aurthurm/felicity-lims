...
class SupervisorSystemUpgrade:
    def __init__(self):
        ...
        self.db_url = "postgresql://user:pass@localhost/beak_lims"
    ...
    async def reload_system(self):
        # Stop the service
        self.supervisor.supervisor.stopProcess("beak")
        # Wait for stop
        await asyncio.sleep(5)
        # Start the service
        self.supervisor.supervisor.startProcess("beak")

        # Verify status
        info = self.supervisor.supervisor.getProcessInfo("beak")
...
"""
# /etc/sudoers.d/beak-upgrade
beak ALL=(ALL) NOPASSWD: /usr/bin/pg_dump
beak ALL=(ALL) NOPASSWD: /usr/bin/psql
beak ALL=(ALL) NOPASSWD: /usr/bin/supervisorctl
"""
