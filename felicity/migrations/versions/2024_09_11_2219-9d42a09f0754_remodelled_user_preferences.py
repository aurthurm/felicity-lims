...
def upgrade():
    ...
    op.create_foreign_key(
        None, "user_preference", "user", ["user_uid"], ["uid"], ondelete="CASCADE"
    )
    op.add_column(
        "user_preference", sa.Column("default_route", sa.String(), nullable=True)
    )
...
def downgrade():
    ...
    op.add_column(
        "user",
        sa.Column("preference_uid", sa.VARCHAR(), autoincrement=False, nullable=True),
    )
    op.create_foreign_key(
    ...
    op.create_index(
        "ix_analysis_result_level", "analysis_result", ["level"], unique=False
    )
