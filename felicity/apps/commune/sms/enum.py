...
class SmsTrigger(StrEnum):
    ...
    BELOW_NORMAL = (
        "below_normal"  # Trigger when result is below normal but above warning
    )
    ABOVE_NORMAL = (
        "above_normal"  # Trigger when result is above normal but below warning
    )
    ...
    ANY_ABNORMAL = (
        "any_abnormal"  # Trigger for any abnormal result (outside normal range)
    )
